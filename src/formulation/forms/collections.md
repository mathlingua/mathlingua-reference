...
Collection forms matter for `member_of` reduction. A type can describe a collection target directly, or an `Enables.from:` capability can expose a cast literal such as `{x_ : x_ is \real} as \set`.
